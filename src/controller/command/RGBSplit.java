 ...
 * for rgb split component.
 */
public class RGBSplit implements ImageOperationsCommand {
  ...
  /**
   * Constructs a new RGBSplit object with the specified tokens.
   *
   ...
   */
  public RGBSplit(String token1, String token2, String token3, String token4) {
    this.token1 = token1;
    this.token2 = token2;
    this.token3 = token3;
    this.token4 = token4;
  ...
  public void executeCommand(ImageEnhancement m) throws IOException {
    m.rgbSplit(token1, token2, token3, token4);
  }
}
